    ...
    case "isDisabled":
      if (!document.getElementById("chrome-envmarker")) {
        ...
        if (document.getElementById("chrome-envmarker")) {
          sendResponse({
            isDisabled:
              document.getElementById("chrome-envmarker").style.display ==
              "none",
          });
        } else {
          sendResponse({ isDisabled: "notSet" });
...
function _addEnvironmentLabel() {
  chrome.storage.sync.get(
    ...
    function (data) {
      ...
      if (
        !PLUGIN_STATE.lastUpdate ||
        data.current_state.last_update > PLUGIN_STATE.lastUpdate
      ) {
        _updateMatchers(
          data.current_state.env_settings,
          data.current_state.last_update
        );
      }
...
function _addMarker(item) {
  ...
  wrapperDiv.setAttribute(
    ...
    opacity: 0.9;
    z-index: 2147483647;
    box-shadow: inset 0px 0px 10px 5px #${item.color.replace("#", "")};
    pointer-events: none;
    height: 100%;
    width: 100%;
	`
  ...
  textDiv.setAttribute(
    "style",
    "margin: 0 45px; line-height: 55px; height: 100%; width: calc(100% - 90px); overflow: hidden;"
  );
  ...
  env_settings.forEach(function (item) {
    if (!!item && !!item.address && item.address.startsWith("regex:")) {
      item.regex = new RegExp(
        item.address.substring("regex:".length, item.address.length)
      );
...
function _setConfig(config) {
  ...
  obj_index = ENV_SETTINGS.findIndex(function (obj) {
    return obj.uuid == config.uuid;
  });
  if (obj_index === -1) {
    // Last try, might be a legacy config file.
    obj_index = ENV_SETTINGS.findIndex(function (obj) {
      return obj.address === config.address;
    });
  }
