...
function save_options() {
  ...
  for (var i = 0; i < configs.length; i++) {
    var uuid =
      (configs[i].querySelector(".uuid") &&
        configs[i].querySelector(".uuid").value) ||
      uuidv4();
    var name =
      configs[i].querySelector(".name") &&
      configs[i].querySelector(".name").value;
    var address =
      configs[i].querySelector(".address") &&
      configs[i].querySelector(".address").value;
    var color =
      configs[i].querySelector(".color") &&
      configs[i].querySelector(".color").value;

    if (uuid && name && address && color) {
      var obj = {
        uuid: uuid,
        name: name,
        address: address,
  ...
  var hosted_file =
    document.getElementById("hosted-file") &&
    document.getElementById("hosted-file").value;
  var auto_import =
    document.getElementById("auto-import") &&
  ...
  chrome.storage.sync.get(
    ...
    function (data) {
      ...
      for (var i = 0; i < items.env_settings.length; i++) {
        ...
        template.innerHTML =
          '<tr><td><input class="uuid" type="hidden" value="' +
          uuid +
          '"><input class="name" value="' +
          items.env_settings[i].name +
          '" /></td><td><input class="address" value="' +
...
function add_more() {
  ...
  template.innerHTML =
    '<tr><td><input class="uuid" type="hidden" value="' +
    uuidv4() +
    '"><input class="name" /></td><td><input class="address" /></td><td><input class="color jscolor" value="' +
    ...
    function (data) {
      ...
      a.setAttribute(
        ...
        "data:text/plain;charset=utf-u," +
          encodeURIComponent(JSON.stringify(items.env_settings))
      );
...
function importHostedFile() {
  ...
  if (_validateURL(url)) {
    fetch(url)
      ...
      .catch((err) => {
        alert(
          "Something went wrong when trying to retrieve the file from the URL."
        );
...
document
  ...
  .addEventListener("click", importHostedFile);
